...
- 当初、誤って別リポジトリ `/Users/ktym/git/var-catalog-codex` の中で作業を始めた
- そのリポジトリの内容を確認したところ、静的サイト生成用の Ruby 中心の小規模構成であり、MCP サーバや TogoDX 関連の既存コードは含まれていなかった
- 依存管理ファイルとして `package.json` や `Gemfile` は見当たらず、Node.js は `v22.17.1`、Ruby は `2.6.10` が利用可能であることを確認した
...
- `config/togodx-mcp.example.json`
  - `baseUrl`: `https://togodx.dbcls.jp/api/human`
  - `dataset`: `human`
